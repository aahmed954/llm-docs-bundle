...
prompt = "${St(e)??It}"
image = pipe(prompt).images[0]`],Di=e=>[`from diffusers import DiffusionPipeline

pipe = DiffusionPipeline.from_pretrained("${Re(e)}")
pipe.load_textual_inversion("${e.id}")`],Ni=e=>e.tags.includes("controlnet")?$i(e):e.tags.includes("lora")?Li(e):e.tags.includes("textual_inversion")?Di(e):Mi(e),Pi=e=>{const a=`# Pipeline for Stable Diffusion 3
...
# Importing as module.
import ${K(e.id)}
nlp = ${K(e.id)}.load()`],Dn=e=>[`from span_marker import SpanMarkerModel

model = SpanMarkerModel.from_pretrained("${e.id}")`],Nn=e=>[`import stanza
